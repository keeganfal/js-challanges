...
export const getHasUppercaseLetters = () => {
  // Write your code here
  for (let i = 0; i <= stringWithUppercaseLetters.length; i++){
    if (stringWithUppercaseLetters[i].toUpperCase() === stringWithUppercaseLetters[i]) {
      return(true)
    }
  }
  return(false)
...
export const convertPascalCaseToSnakeCase = () => {
  ...
  for (let i = 0; i <= pascalCaseVariableName.length; i++) {
    if (pascalCaseVariableName[i].toUpperCase() === pascalCaseVariableName[i]) {
      newStr += pascalCaseVariableName[i] + "_";
    }
    else {
      newStr += pascalCaseVariableName[i];
    }
  }
  return(newStr)
};
