...
// export const removeFalseValues = booleanArr.filter(boolVal => {
//   return boolVal.includes(true);
// }) 

export const removeFalseValues = (booleanArr) => {
  const trueVal = booleanArr.filter((boolean) => boolean);
  return trueVal;
};
...
export const createPercentageList = (numbersArr) => {
  const percentages = numbersArr.map((number) => (number * 100) + "%");
  return percentages;
};
...
export const createListOfPoessessions = (possessionsArr, name) => {
  const newPossessions = possessionsArr.map((possession) => (name + " " +possession));
...
export const convertStringToNumbersArray = (numberString) => {
  const newStr = numberString.split("+").map((number) => (parseInt(number)))
  return newStr;
};
...
export const createOddEvenArray = (numberString) => {
  const newStr = numberString.split("+").map((number) => {
